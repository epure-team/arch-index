...
**Oracle-authoring correction (QA, 2026-09-04).** Rows O-2/O-3 named the operator as the source
spells it in a binding, `( -? )`. That is not its identity: the index is built from the typed tree,
so an operator is stored under its `Ident` name — `-?`, `+?`, `<=` — and the parentheses and spaces
are OCaml binding syntax, not part of the name. Querying `( -? )` is REFUSED; querying `-?` works.
The oracle was wrong, not the measurement, and the mechanism is sounder than the row implied.

**How `may-fail` treats a homonym.** It prints one verdict per function sharing the name — three for
`-?` (tez_repr, time_repr, a test helper), two for `apply_operation` (apply.ml, main.ml). Every
verdict is labelled with the bare name, so the caller must know the order to tell them apart; there
is no module-qualified form (`Main.apply_operation` is REFUSED — the module is a separate column,
not part of the identifier). Read the whole output, never `head -1`: truncating it is how this QA
run first mistook the tool for silently picking one node.
