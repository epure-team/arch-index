...
Validate all fifteen FR and fourteen AC in specs/tezos-call-resolution.md against implementation evidence, including all six CHECK commands after the round2 repair. Exact internal target identities/kinds/refusals are essential; aggregate-only success is insufficient. Treat missing baseline/corpus/hash/DB/caller/target and incomplete reports as setup failure, forbidden transitions as assertion failure, zero-gain replay as neutral not keep. Confirm both slices independently, duplicate-row preservation, source/target witnesses, existing target retention and no unjustified MUST/TOP changes.

Positive permitted relation gain plus native/full/review/QA/exact-head hosted CI is required to retain and ship. The comparator alone cannot certify semantic correctness. Do not modify Tezos or widen corpus. Keep owned baseline evidence for remaining attempts; cleanup only owned transient runner outputs after durable reports. Native/corpus checks require no new worktree. The separate pristine CI recalibration diagnostic uses a necessary owned detached checkout and the existing script's two temporary builds; inspect and remove only those newly created worktrees afterwards, retaining the raw result.
