...
Validate all fifteen FR and thirteen AC in specs/tezos-call-resolution.md against implementation evidence, including all five CHECK commands after the round1 repair. Exact internal target identities/kinds/refusals are essential; aggregate-only success is insufficient. Treat missing baseline/corpus/hash/DB/caller/target and incomplete reports as setup failure, forbidden transitions as assertion failure, zero-gain replay as neutral not keep. Confirm both slices independently, duplicate-row preservation, source/target witnesses, existing target retention and no unjustified MUST/TOP changes.
