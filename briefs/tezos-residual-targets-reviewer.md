# Reviewer — tezos-residual-targets
...
only the unchanged assertion's exact line/lambda coordinate replacement
(1509 to1557, then1559 after the bounded repair); no additional entry, rule change or count relaxation.
...
Independent review is mandatory after implementation commit. Audit owned-member
provenance/consumer distinction first, then exact arity and point-free oracles,
frozen predecessor validation and independent alias/body witness class. Verify
new refusal tests nonvacuously and all17 FR/13 AC. Claims metadata draft only.
Do not infer soundness from target counts or accept model consensus as witness.
Exact full guard and all5 checks must run; source-state hashing must not race
another writer. Record actual failures and let convergence gates route repairs.
