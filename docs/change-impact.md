...
| `impact_input` | versioned object | exact already-parsed input: `diff` plus effective range and path-sorted changed new-file lines, or `files` plus `"whole"`. A deletion-only path is retained as `[]`, not confused with whole-file input. It is invocation provenance, not a Git freshness or content claim. |
| `index_provenance` | versioned object | observed schema version, producer runs and analysis-coverage rows, encoded by the same reader as `arch-report`. Empty/null values state absence in an older/flat index; they do not assert completeness. A future recurring consumer can compare stable producer identity and coverage while retaining the path-dependent invocation digest as provenance only. |
