...
| OCaml CMT (`arch-callgraph-ocaml`) | ✅ execution-sound | Each function body — and each promoted lambda — is lowered to a real per-node **CFG** (`arch_index_cfg.ml`); a call is `MUST` iff its block post-dominates the node's entry AND the head resolves uniquely AND the application is saturated. Conditional/partial calls to resolved callees → `MAY_ENUMERATED`; unknowable targets → `MAY_TOP`. Resolution is `Ident`-stamp-based (shadows never forge a `MUST`). A cross-module name that two compilation units could both provide (two libraries each with an `api.ml`, both defining `run`) is bound to the whole **candidate set** — one `MAY_ENUMERATED` edge per candidate, never one guess and never `MUST` — since nothing in the `.cmt` distinguishes them and the source layout is a convention, not a guarantee. |
