...
into ⊤ on this repository **without changing which edges resolve** — comparing resolutions by
name (rowids shift when the schema changes, and lambda nodes are named by line, so both make an
id- or line-keyed diff meaningless), the non-lambda resolved set is unchanged against
`origin/main`. Those edges were previously indistinguishable from external calls; they now read
as unknown instead of as absent, which is the honest cost. Remaining precision follow-up: 0-CFA
