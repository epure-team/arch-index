...
| `producer_run_id` | INTEGER | (`1.4`, roadmap 1.2 / ADR 002) `REFERENCES producer_runs(id) ON DELETE SET NULL` — this edge's provenance as a join rather than denormalised columns per row. Nullable: `producer_runs` is written **only** by the main-schema CMT writer, so an edge from another writer carries NULL |
| `top_reason` | TEXT | (`1.7`, vocabulary widened in `1.9`) why this edge is ⊤ — meaningful only on `MAY_TOP`. See `docs/edge-kind-contract.md` |
| `top_anchor` | TEXT | (`1.7`) the syntactic anchor the ⊤ verdict is attributed to |
| `edge_form` | TEXT | (`1.10`) `'value_alias'` when this row came from a point-free binding (`let f = M.g`) rather than from an application; NULL for every ordinary call. **Orthogonal to `kind`** — an alias is `MAY_ENUMERATED` like any other bounded candidate. What it records is that *no call happens at that site*, which is what a caller-count consumer needs: `fan-in`, `god-modules` and `callers-of` exclude it, because a "most-called" ranking — or a list of callers — that counts re-exports as callers measures the façade, not the code. `reachable-from`/`callees-of` deliberately do **not** exclude it: those ask what running this could reach, and an alias does forward a body. As of `1.11` the vocabulary has a second member, `'module_alias'` (`specs/reexport-resolution.md`): a real call whose head was *spelled* through a module alias. The two demote identically but mean opposite things for a caller count — exclude `'value_alias'`, **count** `'module_alias'`. So the predicate is `COALESCE(edge_form,'') <> 'value_alias'`, never `edge_form IS NULL`. `CHECK(edge_form IS NULL OR edge_form IN ('value_alias','module_alias'))` — a closed vocabulary. See `specs/point-free-aliases.md`, `specs/reexport-resolution.md` |
| `created_at` | TEXT | `DEFAULT CURRENT_TIMESTAMP` — insertion wall-clock. Bookkeeping only: no query reads it, and it is **not** provenance (use `producer_run_id` for that). Present since `1.0` |
...
| `1.6` | *(never shipped — reserved for error-channels while it was in flight, then superseded. The branch landed as `1.8` after `1.7` merged first: a version LOWER than main's while carrying strictly more schema would break every `>=` comparison. Do not reuse this number — a later `1.6` would be a superset of `1.7` numbered below it.)* | — |
...
| `1.8` | `exn_origins.channel`, `exn_scopes.channel`, `exn_edges`, `channel_carriers` — error-channels (specs/error-channels.md). Slices 0-1: re-tag only, no behaviour change (producer emitted only `channel = 'exception'` rows). Slice 2 (spine): the producer additionally writes `result`/`option` value-channel rows through the SAME tables/columns (`channel_carriers` new, additive within this version — a node's c-carrier marker). Slices 2b-3 (polymorphic-variant errors, binds/transforms/converters/alias-chains/`inferred_bind`): no new tables/columns — `exn_origins.form`'s CHECK widened to also accept `'inferred_bind'` (an undeclared bind-shaped operator's ⊤ witness); still additive within this version. Review round 2: `call_exn_scopes`'s key widened from `PRIMARY KEY (call_id)` to `PRIMARY KEY (call_id, scope_id)`, so one call site can carry an exception-channel scope AND a value-channel one instead of the exception one displacing the other. Not a separate version: `1.8` has not shipped, this is part of its own definition, and every table it touches is dropped and recreated on each run (`Arch_index_support.schema_tables_to_drop`), so there is nothing to migrate | `architecture-schema.sql` (in place; no separate migration file) |
| `1.9` | `calls.top_reason` vocabulary gains `'ambiguous_unit'` — roadmap Phase 1 item 1.6. The reference names a unit that IS in this index, but more than one DISTINCT function answers to it and no `.cmt` says which the caller linked against. Deliberately NOT the external case (a root naming no indexed unit at all is NOT degraded to ⊤ by this member — it keeps its `MUST`. That is a statement about ambiguity only: such a reference may still acquire a `callee_id` pointing at an unrelated local module of the same basename, a defect that predates this version and is pinned by scenarios F and J of `tezt/tests/qualified_library_scoping.ml`); collapsing the two is what inflated the abandoned `rebase/sound-qual` branch's repo-wide `MAY_TOP` from 660 to 875. Additive: one new member in an existing CHECK, no new table or column, and `calls` is dropped and recreated each run, so nothing to migrate | `architecture-schema.sql` (CHECK widened in place); `bin/arch_load` accepts the new member |
| `1.10` | `calls.edge_form` — point-free value aliases (`specs/point-free-aliases.md`). One nullable column on `calls` with a closed one-member vocabulary (`CHECK(edge_form IS NULL OR edge_form = 'value_alias')`), marking a row that came from a binding rather than an application. Same additive shape as `1.7`, and versioned for the same reason: `fan-in`/`god-modules`/`callers-of` now answer a *different number* on a database that has the column than on one that does not, and a consumer comparing two runs must have a version to refuse against rather than silently attribute the drop to the code. The producer gates on `Arch_db.has_col` so an older database still answers — that gate is a courtesy to old data, **not** a substitute for this row (#51 part 1). Note `1.10` follows `1.9`: the component is an integer, not a decimal digit — `parse_schema_version` splits on `.` and compares `(major, minor)` as ints, so `1.10 > 1.9` | `architecture-schema.sql` (additive column); `bin/arch_load`'s NDJSON contract is **unchanged** — the loader neither reads nor writes `edge_form`, so its independent version stays `1.2` |
| `1.11` | `calls.edge_form`'s CHECK widened to `CHECK(edge_form IS NULL OR edge_form IN ('value_alias','module_alias'))` — a head spelled through a module alias (`S.f` where the file declares `module S = Saturation_repr`) is rewritten by the producer to the qualified name it denotes and marked (`specs/reexport-resolution.md`). Additive: the constraint accepts strictly more, so no existing consumer's query needs modifying — but a `1.10` database **rejects the new value on insert**, which is what the version is for. The two members demote identically in the kind matrix and mean different things: `'value_alias'` says *no call happens at this site*, `'module_alias'` says *a real call whose head was spelled through a hub*. A caller-count consumer must exclude the first and **count** the second — the predicate is `COALESCE(edge_form,'') <> 'value_alias'`, and writing it as `edge_form IS NULL` (correct while the vocabulary had one member) silently drops 3 247 genuine edges on proto_alpha (500 `.cmt`, `0982a42`, `--errors-profile=tezos`) | `architecture-schema.sql` (CHECK widened in place); `bin/arch_load` unchanged, it neither reads nor writes `edge_form` |
...
| `1.13` | Nullable `exn_origins.operand_*` columns retain typed-AST divisor context for native integer division/remainder origins. Metadata is evidence only and does not change identity, escapes, allow matching or verdicts. | `architecture-schema.sql`, `docs/reporting.md` |
| `1.14` | `functor_catalogue_runs`, `functor_catalogue_inputs`, and `functor_applications` persist selected-CMT syntactic module-application inventory. They are additive provenance only: no function/call edge, target-resolution, runtime-instance, or closed-world semantics change. | `architecture-schema.sql`, `docs/functor-catalogue.md` |
| `1.15` | `functor_binding_inputs`, `functor_declarations`, and `functor_bindings` add artifact-local formal provenance with independent completion. No call-target resolution, substitution, runtime identity or closure is implied. | `specs/functor-binding-resolution.md` |
| `1.16` | `functor_target_inputs` and `functor_target_witnesses` add validated artifact-local proof chains from matched local functor actuals to additive `MAY_ENUMERATED` member targets. The independent `MAY_TOP/module_param` frontier remains; no `MUST`, closed-world result, body clone, or flat-schema inference is implied. | `specs/ocaml-functor-targets.md` |
| `1.17` | `functor_target_occurrences` and `functor_target_candidates` make each physical formal-member site and its actual/member-to-candidate binding durable. Target calls, occurrences, candidates and witnesses publish atomically under `functor_target_contract=v2`; failed publication leaves no usable functor-derived call. | `specs/ocaml-functor-targets.md` |
| `1.18` | `exn_origins.form` gains `assert_false`, separating unconditional `assert false` from a checked assertion. The closed vocabulary changes, so a pre-1.18 database would reject the producer's new row; consumers can also select the unconditional case without text heuristics. | `architecture-schema.sql`, `specs/exn-raise-sets.md` |
...
| `1.2` | `calls.edge_form` — point-free value aliases (`specs/point-free-aliases.md`), the same column as the main schema's `1.10`. This schema gets it because its OCaml `.cmt` branch (`call_graph_extractor.ml`) shares `collect_calls_from_expr` with the main indexer, so alias-marked edges arrive here whether or not the column exists — without it they would land unmarked and `fan-in` would count them. Independent numbering: `1.2` here and `1.10` there describe structurally incomparable schemas and must never be conflated (see the note on `current_flat_schema_version`). |
| `1.3` | `calls.edge_form` gains the member `'module_alias'` — a head spelled through a module alias (`specs/reexport-resolution.md`). This schema has **no CHECK constraint** on the column, so no DDL text changes; it is versioned anyway, for the reason `1.1 → 1.2` was taken — `fan-in`/`god-modules`/`callers-of` answer a *different number* depending on whether the second member is present, and a consumer comparing two runs must have a version to refuse against. **A version records what a consumer can observe, not what the DDL says.** The flat producer emits it because `call_graph_extractor.ml` shares `collect_calls_from_expr` with the main indexer. |
