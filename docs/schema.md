...
| `edge_form` | TEXT | (`1.10`) `'value_alias'` when this row came from a point-free binding (`let f = M.g`) rather than from an application; NULL for every ordinary call. **Orthogonal to `kind`** — an alias is `MAY_ENUMERATED` like any other bounded candidate. What it records is that *no call happens at that site*, which is what a caller-count consumer needs: `fan-in`, `god-modules` and `callers-of` exclude it, because a "most-called" ranking — or a list of callers — that counts re-exports as callers measures the façade, not the code. `reachable-from`/`callees-of` deliberately do **not** exclude it: those ask what running this could reach, and an alias does forward a body. `CHECK(edge_form IS NULL OR edge_form = 'value_alias')` — a closed vocabulary. See `specs/point-free-aliases.md` |
