...
(** Check whether a function_effects row already exists for this function name. *)
let fn_exists db name =
  match Sqlite3.prepare db
    "SELECT 1 FROM function_effects WHERE function_name=? LIMIT 1" with
  ...
  if not (Sys.file_exists db_path) then
    ...
    let update_st = Sqlite3.prepare db
      "UPDATE function_effects SET \
         ...
         gating             = COALESCE(?, gating), \
         value_touched      = COALESCE(?, value_touched), \
