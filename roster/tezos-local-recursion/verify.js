...
function candidate(witnessFile) {
  ...
  const output=fs.mkdtempSync(path.join(b.ROOT,'improvement/2026-09-14-tezos-resolution/attempt4-candidate-'));
  const temp=fs.mkdtempSync(path.join(os.tmpdir(),'arch-index-local-recursive-candidate-'));
  try {
