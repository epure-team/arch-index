...
const reviewedWitness = path.join(root,
  'improvement/2026-09-14-tezos-resolution/attempt5-reviewed-witness.json');
...
function run() {
  ...
  if(!fs.existsSync(reviewedWitness))throw new Error(`reviewed fixed410 witness missing: ${reviewedWitness}`);
  if(!fs.existsSync(path.join(__dirname,'verify.js')))throw new Error('candidate verifier missing');
