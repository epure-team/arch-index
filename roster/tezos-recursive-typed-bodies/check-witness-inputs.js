...
function main(){const temp=fs.mkdtempSync(path.join(os.tmpdir(),'arch-index-recursive-open-witness-inputs-'));try{
  const source=path.join(temp,'fixture.ml');fs.writeFileSync(source,`module M = struct end\nlet outer n = let rec aux = let open M in fun x -> if x=0 then 0 else aux (x-1) in aux n\nlet over n = let rec aux = let open M in fun x -> if x=0 then (fun y -> y) else (fun y -> (aux (x-1) y) + (aux (x-1) y)) in aux n 1\n`);compile(temp,source);
  ...
  const witness={schema_version:'local-recursive-open-witness-v1',baseline_digest:baseline.digest,candidate_digest:candidate.digest,evidence_file:evidence,evidence_sha256:fileSha256(evidence),probe_sha256:fileSha256(path.join(__dirname,'recursive-witness.ml')),
  ...
  rejectNative((r,a)=>{a.matching_recursive_scope.direct_tfunction_rhs=true;},'direct-only historical shape');
  rejectNative((r,a)=>{a.matching_recursive_scope.single_path_open_function_rhs=false;},'missing exact open shape');
  rejectNative((r,a)=>{a.matching_recursive_scope.open_operand_constructor='Tmod_apply';},'computed module operand');
  rejectNative((r,a)=>{a.matching_recursive_scope.inner_function_loc=a.matching_recursive_scope.rhs_loc;},'outer wrapper is not physical inner function');
  ...
  console.log('PASS recursive open native witness inputs (genuine CMT, two overapplied heads, independent head/residual reuse refusals)');
