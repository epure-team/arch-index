...
function prepare(candidate,nativeFile,output){
  ...
  if(path.basename(nativeFile)!=='attempt5-native-410.json')fail('native evidence must be attempt5-native-410.json');
  ...
  const witness={schema_version:'local-recursive-open-witness-v1',baseline_digest:provenance.baseline_digest,
...
if(require.main===module){try{if(process.argv.length!==5)fail('usage: prepare-witness.js CANDIDATE_DIR ATTEMPT5_NATIVE_410_JSON OUTPUT_DRAFT');
