(* Compiler-native witness for singleton local path-open recursive self-heads.
...
type recursive_scope = {
  ...
  rs_open_function_body : expression option;
}

let expected_root rs =
  Option.value ~default:rs.rs_binding.rhs rs.rs_open_function_body
...
let process file =
        ...
        (fun self e ->
                    ...
                    | Tpat_var (id, _, binder_uid) ->
                        ...
                        Some
                          { rs_binding = b; rs_group_size = group_size;
                            ...
                            rs_open_function_body =
                              (match vb.vb_expr.exp_desc with
                               | Texp_open (od, ({exp_desc = Texp_function _; _} as inner)) ->
                                   (match od.open_expr.mod_desc with Tmod_ident _ -> Some inner | _ -> None)
                               | _ -> None) }
                    | _ -> None)
                  vbs
              in
          ...
          | Texp_function _ ->
              ...
              if leading then default.expr self e
                ...
                let root_scope =
                  List.find_opt
                    (fun rs -> (rs.rs_direct_function_rhs || Option.is_some rs.rs_open_function_body)
                               && expected_root rs == e)
                    !current_recursive
                in
  ...
  let root_owner rs =
    List.filter
      (fun o -> o.owner_kind = "recursive_root_literal" && o.owner_expr == expected_root rs)
      owners
  in
  ...
  let classification a =
    ...
    | Some rs, Some _ when Option.is_none rs.rs_open_function_body ->
        if rs.rs_direct_function_rhs then "refuse_direct_function_already_supported"
        else "refuse_non_path_open_function_rhs"
    | Some rs, Some _ ->
  ...
  let scope_json rs =
    ...
    obj
      [ ("binder", ident rs.rs_binding.binder); ("binder_uid", uid rs.rs_binding.binder_uid);
        ...
        ("single_path_open_function_rhs", bool (Option.is_some rs.rs_open_function_body));
        ("inner_function_loc", opt (fun e -> loc e.exp_loc) rs.rs_open_function_body);
        ("open_operand_constructor", if Option.is_some rs.rs_open_function_body then quote "Tmod_ident" else "null");
        ("open_module_path",
          match rs.rs_binding.rhs.exp_desc with
          | Texp_open (od, _) ->
              (match od.open_expr.mod_desc with Tmod_ident (p, _) -> quote (Path.name p) | _ -> "null")
          | _ -> "null");
  ...
  let application_json a =
    ...
    obj
      [ ("native_occurrence_index", string_of_int a.app_index);
        ...
        ("root_syntactic_arity", opt (fun rs -> string_of_int (fn_arity (expected_root rs))) (matching_scope a));
  ...
  in
  obj
    [ ("schema_version", quote "recursive-open-witness-v1"); ("cmt", quote file);
