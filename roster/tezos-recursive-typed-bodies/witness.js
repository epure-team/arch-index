...
function validateRecord(record){
  if(!record||record.schema_version!=='recursive-open-witness-v1'||record.artifact_digest_algorithm!=='md5'
...
function validateNativePair(record,locator,before,after){
  ...
  if(!scope||scope.group_size!==1||scope.direct_tfunction_rhs!==false||scope.single_path_open_function_rhs!==true
    ||scope.open_operand_constructor!=='Tmod_ident'||typeof scope.open_module_path!=='string'
    ||!scope.open_module_path)fail('singleton exact path-open function evidence missing');
  if(!root||scope.root_observation_count!==1||root.kind!=='recursive_root_literal'||!validLoc(scope.rhs_loc)
    ||!validLoc(root.loc)||!sameLoc(scope.inner_function_loc,root.loc)
    ||!contains(scope.rhs_loc,scope.inner_function_loc)
...
function loadWitness(file,baseline,candidateSnapshot,manifest){
 try{
  ...
  if(witness?.schema_version!=='local-recursive-open-witness-v1')fail('witness schema refused');
  ...
  if(!Array.isArray(manifest?.records)||cmts.length!==manifest.records.length
    ||new Set(manifest.records.map(entry=>entry[2])).size!==cmts.length
    ||manifest.records.some(entry=>!cmts.includes(entry[2])))fail('native evidence must cover the complete input manifest');
