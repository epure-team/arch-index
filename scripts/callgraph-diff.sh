...
dump() { # (caller_name, callee_name, call_site, kind) sorted
  sqlite3 "$1" "SELECT f.name||'|'||c.callee_name||'|'||c.call_site||'|'||c.kind
                FROM calls c JOIN functions f ON c.caller_id=f.id ORDER BY 1;"
}
sites() { # population without kind
...
dropped=$(comm -23 /tmp/cgdiff-old-sites.txt /tmp/cgdiff-new-sites.txt)
added=$(comm -13 /tmp/cgdiff-old-sites.txt /tmp/cgdiff-new-sites.txt | wc -l)
