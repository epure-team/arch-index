...
| Q | A |
|---|---|
| Naming: the `exn_` prefix is kept on purpose **[review round 1]** | Every table, column and reason token in this feature keeps the `exn_` prefix it inherited from the exception channel: `exn_origins`, `exn_scopes`, `exn_edges`, `call_exn_scopes`, and the ⊤ reason `unknown_exn_value` — which the solver emits on **every** channel, value channels included, not only `exception`. An earlier draft of this spec wrote `unknown_error_value`; the shipped token is `unknown_exn_value` and this document has been aligned to it. A wholesale `exn_*` → `error_*` rename was considered and rejected: it would churn the schema, the NDJSON record names, every query, `docs/exception-raise-sets.md` and three frozen validation corpora, to buy a prefix. Continuity with the exception channel — the same rows, the same solver, one vocabulary — is worth more than the tidier name. Read `exn_` as "failure", not as "exception". |
...
| Config file vocabulary (`arch-errors.toml`) | `[channel.<name>]`: `type` (carrier constructor path), `underlying = [paths]` (alias targets — the `.cmt` never expands abbreviations, and Tezos prints the underlying as `Tezos_protocol_environment_alpha.Pervasives.result`, so both spellings must match) **[research]**, `error_arg` (1-based), `lift = [paths]`, `error_type` (path of the error type; `""` = identity is the literal constructor's own path), `aliases = [paths]` (other paths under which `type`/`error_type` print — re-exports, environment aliases) **[EC-10]**, `origins = [{path, arg=1}]` (functions/constructors whose LITERAL constructor argument at `arg` names the error; `Stdlib.Error`, `Error_monad.error`, `error_when cond E` → `arg=2`) **[research]**, `binds = [paths]`, `handlers = [{path, arg=1}]` (declared handler applied to the value at `arg`), `transforms = [{path, mode="add"|"replace", arg}]` **[C-3, C-13]**, `converters = [{path, from, to, arg, error="path"|""}]` **[C-12, EC-8]**, `sinks = [paths]` (default `Stdlib.ignore`), `unwrap = [paths]`. `[summaries] "<callee path>" = { <channel> = ["path", …] }`. Composition order: built-ins < profile < user file, keyed by channel name. A later definition of the SAME channel **extends** it field by field — every list-valued field is the base's entries followed by the override's new ones, deduped; `error_arg`/`error_type` are overridden only when the override sets them — so a profile adds vocabulary to a built-in without restating what it is not changing **[C-6, review round 2]**. (It replaced the whole channel until review round 2. That forced `profiles/tezos-errors.toml` to copy six `Stdlib.Option.*` paths in purely to keep them, and, since a restated channel is one the operator wrote, made those copies count against `--errors-strict` — which is what made strict unsatisfiable for the shipped profile.) A same-named `[summaries]` entry is still REPLACED: a summary is a complete statement of one callee's error set, so extending it could never narrow one. |
...
| Config validation | For each channel, every declared path is looked up in the set of paths the walker *saw* in the corpus. The split is load-bearing: **value** paths (`origins`/`handlers`/`binds`/`transforms`/`converters`/`sinks`) are matched against `Texp_ident`/`bop_op_path`/constructor paths, **type** paths (`type`/`underlying`/`aliases`, plus **`lift` and `unwrap`**) against `Tconstr` paths. Filing `lift`/`unwrap` on the value side makes them unmatchable by construction **[review round 1]**. Per-path miss → **warning** printed (`arch-errors: channel c: 'X' matched nothing`) and recorded in `comment_db_meta.error_config_unmatched`; a channel whose `type` (or every `underlying`/`aliases`) matches nothing → **exit 1** (`arch-errors: channel c: carrier type matched nothing in the indexed corpus`) — the "declaration matching nothing" bug class; `--errors-strict` promotes every warning to exit 1 **[C-5]**, *except* a miss on a path the operator's own config FILES do not spell: an inherited built-in path is not the operator's declaration, and holding them to a `Stdlib.*` path their corpus happens not to use is not a bug report about their config **[review rounds 1 and 2]**. The waiver is PER PATH, not per channel — now that a same-named channel EXTENDS a built-in rather than replacing it, a per-channel test would waive nothing the moment a profile adds one bind, dragging every inherited path back into the verdict (which is what happened to `profiles/tezos-errors.toml` in round 1). Note also that `Stdlib.option` is not merely a spelling the compiler declines to print: `stdlib.mli` re-exports `result` but NOT `option`, so it is not a type constructor at all and the built-in no longer declares it **[review round 2]**. |
| Channel reachability **[review round 1, FR-027]** | Carrier selection is first-match-wins over the merged channel list, so a channel all of whose carrier types are already claimed by an **earlier** channel that applies no narrower argument test can never own a carrier. Such a channel MUST be **refused at load time** (exit 1), naming both channels and giving a remedy the operator can actually carry out — when the shadowing channel is a BUILT-IN it cannot be reordered (built-ins are always merged first), so the message MUST instead point at extending it under its own name or adding a distinguishing `error_type` **[review round 2]** — not published in `error_contract`, where it would answer `NOT_A_CARRIER` for every function in the corpus: a positive claim about code that was never examined. Two channels over the same carrier type distinguished by `error_type`/`error_arg` are reachable and MUST NOT be refused. |
...
| Transforms **[C-3, C-13]** | `mode = "add"` (`record_trace`, `trace`, `trace_eval`): result set = inner set ∪ literal argument origins (`arg`), inner marked `wrapped`. `mode = "replace"` (`Result.map_error`): result set = the literal-constructor returns of the mapping function when it is a **lambda literal**, else ⊤ `unknown_exn_value`; the inner set does NOT survive. **Narrowed against the original wording (review round 3):** a *named* mapper whose own set is Known is NOT resolved — it yields ⊤ like any other non-lambda. Resolving it is possible and unimplemented; ⊤ is the sound direction, and the requirement is stated as what ships rather than left as an undocumented divergence. |
| Converters — cross-node limit **[review round 3]** | A converter closes its `from` channel only within the SAME CFG node. A guarded thunk (`catch_f (fun () -> …)`) becomes its own lambda node, and the scope minted on the enclosing node does not reach into it, so the `from` channel can stay ⊤ while the `to` channel is bounded — see oracle row O-5. Architectural, not a rule gap; recorded so the spec states what ships. |
...
| Schema **[C-18]** | `exn_origins.channel TEXT NOT NULL DEFAULT 'exception'`, `exn_scopes.channel` likewise, `call_exn_scopes` gains nothing but its key: `PRIMARY KEY (call_id, scope_id)` rather than `(call_id)`, so one call site can carry one scope PER channel (the scope itself still carries the channel, and a consumer filters there) **[review round 2]**, new `exn_edges(call_id, channel, role ∈ {propagates, bind_arg, sink, transform_add, transform_replace, convert})` (additive; only rows for value channels — the exception channel keeps "every edge"). Producers always rebuild the DB; a consumer opening a DB without the `channel` column (pre-1.3) reads it as exception-only (`has_col`). `current_schema_version` bumped per the rule above (base+1 minor) with its `docs/schema.md` history row. |
...
7. **Given** `let w () = Res.map_error (fun _ -> B 3) (f ())` with `map_error` declared `mode = "replace"`, **Then** `{Ec_a.B}` (inner `A` gone); **Given** `let w2 () = Res.map_error fn (f ())` with `fn` a parameter, **Then** `UNBOUNDED (⊤)` reason `unknown_exn_value`.
...
- **FR-023** [US-1]: Every declared path MUST be checked against the paths the walker saw, on the side it belongs to — `lift`/`unwrap` are TYPE paths, not value paths **[review round 1]**; per-path misses MUST be printed and recorded in `error_config_unmatched`; a channel whose carrier type matches nothing MUST abort with exit 1; `--errors-strict` MUST make any miss fatal, except a miss on a path that none of the loaded config FILES declares (i.e. one inherited from the built-ins) — a PER-PATH waiver, not a per-channel one **[review rounds 1 and 2]**.
- **FR-023b** [US-1] **[review round 1]**: A channel that is structurally unreachable — every carrier type already claimed by an earlier channel applying no narrower argument test — MUST be refused at load time with exit 1 naming both channels, rather than published in `error_contract`. This is a config-only check and MUST run before any `.cmt` is read.
...
#### Producer (US-2)
- **FR-025** [US-2]: For each c-carrier node the producer MUST record origins (literal ordinary/extension constructor → canonical path; literal **polymorphic variant** → its bare label; else ⊤ `unknown_exn_value`), handler scopes (`match … Error/None` on the head call or its single-let alias chain; declared handlers), sinks, binds (bound-call + continuation edges), transforms (mode), converters, each with `channel = c`, in `exn_origins`/`exn_scopes`/`exn_scope_catches`/`call_exn_scopes`/`exn_edges`.
- **FR-026** [US-2]: An arm MUST be closing iff unguarded and the variables bound by its pattern do not occur in its RHS.
- **FR-027** [US-2]: The carrier check MUST follow the Clarifications rule (type/underlying/aliases, error_arg head or type variable, lift stripping). **Narrowing, review round 1:** selection is first-match-wins, so a function is a carrier of at most one channel rather than of each channel it satisfies. FR-023b keeps that narrowing honest — a channel it would silence is refused rather than shipped answering `NOT_A_CARRIER` about code nobody looked at.
