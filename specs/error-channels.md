...
- **FR-023** [US-1]: Every declared path MUST be checked against the paths the walker saw, on the side it belongs to — `lift`/`unwrap` are TYPE paths, not value paths **[review round 1]**; per-path misses MUST be printed and recorded in `error_config_unmatched`; a channel whose carrier type matches nothing MUST abort with exit 1; `--errors-strict` MUST make any miss fatal, except a miss belonging to a channel structurally identical to the built-in of the same name **[review round 1]**.
