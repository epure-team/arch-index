...
| Edge kinds? | `MUST` and `MAY_ENUMERATED` edges propagate `raises(m)`; `MAY_TOP` edges contribute `Top(may_top_edge @ call_site)`; callee `NULL` ⇒ `ext:<name>` leaf ⇒ `Top(external <name>)` unless `<name>` ∈ fixed table {`Stdlib.raise`, `Stdlib.raise_notrace`, `Stdlib.failwith`, `Stdlib.invalid_arg`, `Stdlib.exit`, `Stdlib.Printexc.raise_with_backtrace`} ⇒ ∅ (their effect is already an origin) or `--assume-externals-pure` ⇒ ∅. A dropped node is already `MAY_TOP` in `calls` and reads as `may_top_edge` (no separate reason — amended 2026-09-03). Every ⊤ contribution is subject to `close_S` (a catch-all closes ⊤; a constructor set does not). |
| Lattice / termination? | `Known of PathSet \| Top of PathSet × Reason set` (the resolved part is carried under ⊤ so it is still reported — amended 2026-09-03). `Top` absorbs; reason sets are unions (each reason = kind + one witness). Universe of paths is finite (paths occurring in `exn_origins`/`exn_scope_catches` of the DB); join is monotone; worklist over `MUST ∪ MAY_ENUMERATED` edges terminates (C-6). Dominant reason order for `exn-stats`: `may_top_edge` > `external` > `unknown_exn_value` (C-8). |
...
| Deferred bodies (review finding R1, 2026-09-03) | A raise inside `lazy`, an object method or a functor body that is lexically under a `try` is NOT covered by it: the producer clears the scope stack around deferred walks (`Arch_index_exn.with_cleared_scopes`), mirroring the CFG's isolated blocks. Scenario US-1.11. |
| Functor-body exceptions (review finding R7) | Residual: definition path on the raise side vs application path on the handler side never unify — over-approximating; documented. |
...
11. **Given** `let mk () = try lazy (raise A) with A -> lazy ()`, **When** indexed, **Then** the origin is `escapes=1` with no scope (a deferred body is outside the handler), and `raises --assume-externals-pure use` for `let use () = Lazy.force (mk ())` is `BOUNDED_UNDER_HYP(externals_pure): {A}`.
12. **Given** `raise (Obj.magic 0)`, an or-pattern arm `| A | B -> 0`, an alias arm `| (B _ as e) -> …`, a nested `try (try f () with A -> 0) with B _ -> 1`, a handler on a rebound alias `with Alias -> 0`, `Printexc.raise_with_backtrace (C) bt`, `x / y`, `a.(i)` and a comparison at an abstract type, **When** indexed and queried, **Then** the forms `unknown`/`raise`/`division`/`index`/`compare` are recorded, the or/alias arms catch both paths, the nested chain closes both, the alias handler closes `Not_found`, and `raises` reports `UNBOUNDED (⊤)` with `unknown_exn_value` for the `Obj.magic` raiser while still listing its literal raises.
...
9. **Given** a name with no `functions` row, **When** `raises nosuch`, **Then** exit 3 and the same "unknown function" refusal `unreachable` gives.
...
| C-2 | US-1.4 | `exn_origins.form` vocabulary | `raise \| reraise \| unknown \| failwith \| invalid_arg \| assert \| assert_false \| partial_match \| compare \| division \| index`, CHECK-constrained (one list in the SQL CHECK, `Arch_index_exn.form`, and the doc table — keep in sync). |
...
- **FR-015** [US-2]: `raises` MUST require the ⊤-marking contract (`require_contract`) and a known function (`need_known`), with the same exit codes as `unreachable`: exit 3 for both a missing contract and an unknown name (amended 2026-09-03 — the original text said 2 for an unknown name; `need_known` has always exited 3).
...
- **FR-017** [US-3]: `exn-stats` MUST print node count, bounded/unbounded counts and shares, unbounded split by dominant reason (`may_top_edge > external > unknown_exn_value`), origin/scope/escaping-origin counts, and a hypothesis line when the flag is set.
...
- AC-9 [US-2, FR-015]: unknown name exit 3 (`need_known`); un-⊤-marked DB exit 3 (`REFUSED — … NOT ⊤-marked`).
...
- `raise-set`: the lattice value `Known of PathSet | Top of PathSet × Reason set` computed per node; `Top` reasons ∈ {`may_top_edge`, `external`, `unknown_exn_value`} each with a witness.
