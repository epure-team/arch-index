...
| C-2 | US-1.4 | `exn_origins.form` vocabulary | `raise \| reraise \| unknown \| failwith \| invalid_arg \| assert \| partial_match \| compare \| division \| index`, CHECK-constrained (one list in the SQL CHECK, `Arch_index_exn.form`, and the doc table — keep in sync). |
