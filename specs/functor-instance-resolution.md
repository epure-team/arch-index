...
- **FR-001** [US-1; scenarios 1–3; check: inventory]: For each selected artifact accepted as an OCaml 5.3 `Implementation structure`, the catalogue MUST contain exactly one occurrence for every reachable immediate `Tmod_apply` and `Tmod_apply_unit`, including occurrences below expressions, classes/objects, unpack, module-type-of, functor bodies, nested structures, anonymous arguments, unnamed bindings, and local modules.
