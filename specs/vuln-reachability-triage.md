...
| Can we add a ninth verdict? | **No** — and the reason must be stated so it survives roadmap 4.12, which is in flight and changes the mechanism. **The stable reason:** the vocabulary carries an invariant that every member appears on every report, zero included (`arch_report.ml`), and the census is asserted to sum to the rule count. A ninth value breaks a total that consumers are promised. That holds however the vocabulary is represented. **The unstable reason, stated with its scope:** on `main` at `090f832` the vocabulary is a hand-written string list and `failing` is a disjunction of string equalities, so a value not named there evaluates `false`. **Measured, not assumed:** the vocabulary holds 8, `failing` names 7, and the omitted one is `PASS` — which must not fail. So coverage is **complete** and the gap is **latent**, not a live fail-open. **STATUS CORRECTION, 2026-09-06 — roadmap 4.12 has SHIPPED.** It merged as PR #92 (`main` at `b120765`), so the sentence that follows describing it as unpushed with no PR is stale, and so is the paragraph above scoped to `main` at `090f832`: on today's `main` the vocabulary is no longer a hand-written string list and `failing` is no longer a disjunction of string equalities. Those two passages are kept rather than rewritten because each states its own scope, which is what let this correction be a status note instead of a re-derivation. **The residual below was re-checked against the merged code, not carried over, and it still holds exactly as written** — `all` is hand-written, OCaml cannot enumerate a variant without a ppx, and a constructor omitted from it still compiles while the census assertion dies loudly. One thing the residual does not mention and the merged code does: review found a fourth place to forget, a hand-written `verdict_of_string` carrying `| _ -> None`, and that case was removed by deriving it via search over `all` rather than documented. Roadmap 4.12 (branch `fix/verdict-failing-exhaustive`) makes the type a sum, derives the vocabulary from it, and makes `failing` exhaustive — a ninth constructor becomes a compile error rather than a silent pass. **Its own residual, which this spec must not paper over:** the enumeration list is still hand-written (OCaml cannot enumerate a variant without a ppx), so a constructor omitted from it still compiles, the census stops summing, and the existing assertion dies **loudly — not impossibly**. Do not assume the vocabulary is compile-time total in every direction. |
