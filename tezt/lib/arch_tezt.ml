...
(* [Cli_paths] is generated by a rule in tezt/lib/dune whose [deps] are the CLI
   executables this suite drives. Referencing it here puts those executables in
   the build graph of anything that links [arch_tezt], including a scoped
   `dune build tezt/tests/main.exe`. See that rule's comment for the
   measurement. *)
let () = ignore (Cli_paths.built : bool)
